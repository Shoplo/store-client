...
class Order extends Resource
{
    public function retrieve($id = 0, $params = array(), $cache = false)
    {
        if ($id == 0) {
            if (!$cache || !isset($this->bucket['order'])) {
                ...
                $result = empty($params) ? $this->send($this->prefix."orders") : $this->send(
                    $this->prefix."orders?".$params
                );
    ...
    {
        $params = $this->prepare_params($params);
        $result = empty($params) ? $this->send($this->prefix."orders/{$id}/", 'DELETE') : $this->send(
            $this->prefix."orders/{$id}/?".$params,
            'DELETE'
        );
