    ...
    public function retrieve($id = 0, $params = array(), $cache = false)
    {
        if ($id == 0) {
            if (!$cache || !isset($this->bucket['customer'])) {
                ...
                $result = empty($params) ? $this->send($this->prefix."customers") : $this->send(
                    $this->prefix."customers?".$params
                );
            ...
            if (!$cache || !isset($this->bucket['customer'][$id])) {
                $params = $this->prepare_params($params);
                $result = empty($params) ? $this->send($this->prefix."/customers/".$id) : $this->send(
                    $this->prefix."/customers/".$id."?".$params
                );
